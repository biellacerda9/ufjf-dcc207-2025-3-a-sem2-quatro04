...
export default function Button({ texto, status = 'pendente' , textoAtivo = "Tomado", tipo, onClick}: ButtonProps) {
  ...
  if (status === 'excluir') {
    return (
      <button onClick={onClick} className='bg-red-600 text-white font-semibold text-lg shadow-md hover:bg-red-700 rounded-full transition w-6 h-6 flex items-center justify-center'> X </button>
    )
  }
